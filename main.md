...
### v0.1.28 — 2026-09-12
- **Storage security hardening** — закрыт один из пунктов "B" из production-readiness чеклиста (v0.1.27): `menu-images`/`promo-images` bucket'ы принимали файл любого типа/размера. RLS (кто пишет — только `is_admin()`) не менялась и уже была корректна; добавлено ограничение ЧТО можно писать.
- **Новая идемпотентная миграция `20260912100000_storage_upload_limits.sql`** — `update storage.buckets set file_size_limit = 5242880, allowed_mime_types = array[...]` на оба существующих bucket'а (не создаёт новых, не переписывает `20260911160000`/`20260911180000`). Лимит 5 МБ — с запасом под неотредактированное фото с телефона (текущее использование — превью ≤80px в админке, ≤1200px на публичных страницах), при этом ограничивает worst-case злоупотребление. MIME: `image/jpeg`, `image/png`, `image/webp`, `image/avif`.
- **Проверено перед написанием миграции, что она не сломает текущую загрузку**: оба `ImageUpload.tsx` вызывают `supabase.storage.from(...).upload(path, file, ...)`, передавая браузерный `File` напрямую — `@supabase/storage-js` в этом случае заворачивает его в `FormData` (не в `text/plain` по дефолту, который используется только для raw-строк/потоков), так что реальный `file.type` браузера доходит до Storage backend как есть. Существующие уже загруженные объекты не затрагиваются — Supabase Storage применяет `file_size_limit`/`allowed_mime_types` только к новым upload/update запросам, не ретроактивно.
- **`app/admin/menu/ImageUpload.tsx` и `app/admin/promotions/ImageUpload.tsx`** — добавлена client-side проверка (UX-подсказка, не защита): `accept` инпута сужен с `image/*` до точного списка 4 форматов; до отправки в Storage проверяются `file.type` (понятная ошибка «Неподдерживаемый формат…») и `file.size` (понятная ошибка «Файл слишком большой. Максимум 5 МБ.») — реальное ограничение остаётся в Storage, эти проверки только избавляют от лишнего round-trip к серверу на явно невалидном файле.
- Попутно (та же пара файлов, тот же повод) — `app/admin/promotions/ImageUpload.tsx:71` `alt=""` заменён на `alt="Текущее изображение"` — пропущен в аудите v0.1.27 (тот форк проверил `admin/menu/ImageUpload.tsx`, но не нашёл отдельный promotions-файл).
- **Не тронуто по прямой инструкции**: payment provider, `order_items_admin_manage`/`_delete` policy, работающая auth/orders/reservations-логика.
- Проверено: `npm run build`/`lint` чистые. **Миграция ещё не применена к живой БД** — ручной шаг для владельца, как и все предыдущие миграции в этом проекте (см. `gastromania-tasks.md`, «Ручной шаг» в конце Блока 14).

### v0.1.27 — 2026-09-12
- **Production-readiness audit** всего продукта (14 маршрутов, публика + admin) — навигация/auth/RLS, честность контента, media, брони/заказы, SEO/accessibility. Payment provider не подключался. Полный список находок — в отдельном отчёте владельцу; здесь только то, что реально исправлено.
- **Найден и исправлен реальный content-баг: `Reservation.tsx` всё ещё содержал остатки вымышленного копенгагенского концепта**, пропущенные в проходе v0.1.21 (тот трогал Contact/Footer/Hero/Chef/Story, но не Reservation): цены дегустационного меню в датских кронах («2 800 DKK», «1 400 DKK»), заголовок «Столик **на восемнадцать**» (вымышленная вместимость, противоречит реальному лимиту в 6 гостей чуть ниже) и упоминание закрытых дней («со вторника по субботу»), не подтверждённое владельцем и оптически спорящее с реальными часами «Круглосуточно» (v0.1.25). Убрал блок `details` (цены/длительность/дресс-код — ни один факт не подтверждён), переписал заголовок на «Столик / для вас / уже ждёт.», убрал упоминание конкретных дней недели (оставил только подтверждённые факты: начало в 19:00, до 6 гостей).
- **Найдены и исправлены 5 файлов с забытой датой основания «С 2018 года»** в дублирующихся header-блоках (`login`, `privacy`, `checkout`, `delivery`, `account/orders`) — заменено на «Москва · м. Дубровка», как в Navigation/Footer.
- **`app/privacy/page.tsx`** — фраза «если в будущем появится личный кабинет» была устаревшей (личный кабинет и привязка брони к аккаунту через `profile_id` уже реализованы, см. `reservation/actions.ts`) — переписана в настоящем времени.
- 3 мелких `alt=""` в admin (`admin/menu/page.tsx`, `admin/promotions/page.tsx`, `admin/menu/ImageUpload.tsx`) заменены на осмысленный alt.
- `Footer.tsx` — контраст ссылки «Конфиденциальность» поднят с `/25` до `/55` (WCAG AA для единственной функциональной legal-ссылки в футере).
- **Осознанно НЕ тронуто (архитектура, зафиксировано для отдельного решения):** Storage bucket'ы `menu-images`/`promo-images` созданы без `file_size_limit`/`allowed_mime_types`; `order_items_admin_manage`/`_delete` дают admin прямой UPDATE/DELETE на `order_items` в обход `create_order()` (не эксплуатируется ни одним текущим UI, но теоретически может рассинхронизировать `subtotal`); `metadataBase`/canonical не заданы (нужен реальный production-домен, инвентировать нельзя); OG-`image` отсутствует (нет реального фото).
- Проверено: `npm run build`/`lint` чистые. Полный production-readiness чеклист (A/B/C/D/E) — в отчёте владельцу и в `gastromania-tasks.md`.

### v0.1.26 — 2026-09-12
- **`Story.tsx` — блок «Ритуал и Место» переделан с 3 broken-image Unsplash-плейсхолдеров на реальное видео Gastromania.** Левый большой тайл «Кухня Gastromania» (`aspect-[16/10]`, col-span-2) — фрагмент `КУХНЯ_ШЕФ_Повар 1.mp4` 0.0–9.0с (9с, общий план работы кухни: огонь, нарезка, шеф, разлив масла). Правый верхний «Сезонные ингредиенты» — фрагмент `Десерты_драфт.mp4` 4.0–9.0с (5с, крупные планы кремовой отсадки/ягод). Правый нижний «Процесс приготовления» — отдельный, не пересекающийся по кадрам фрагмент того же `КУХНЯ_ШЕФ_Повар 1.mp4` 9.0–14.0с (5с, шпажка с гарниром, огонь, соус). Ни один фрагмент не повторён между тремя тайлами. Все клипы — H.264/`yuv420p`, `scale=720:1280`, `crf 26`, `faststart`, без звука (`-an`), + свой JPG-постер (для «Процесс приготовления» постер подобран вручную на кадре с огнём — midpoint попадал на кадр с сырым мясом крупным планом, для статичного превью неаппетитно).
- Добавлен всегда видимый (не только по hover) градиентный overlay + подпись на каждом тайле — раньше в этом блоке подписей не было вообще, только `alt`-текст. Grid-композиция не менялась (тот же `grid lg:grid-cols-3` + `flex-col` для правой колонки).
- **Найден и исправлен баг с мобильной вёрсткой этого блока** (не только новый — тот же паттерн был и в старой Image-версии): на mobile правая колонка не растягивалась под высоту левого тайла (grid `align-items:stretch` работает только внутри одной grid-строки, а на мобильном экране колонка уже не в одной строке с большим тайлом), из-за чего `flex-1`-дети без явной высоты схлопывались почти до нуля — второй и третий тайл существовали в DOM, но были практически невидимы. Исправлено — `aspect-[16/10] lg:aspect-auto` на этих двух тайлах, задаёт реальную высоту на мобильном и не мешает `flex-1` на десктопе.
- Ленивая загрузка — тот же `{imgInView && <video>}` (переиспользован существующий `useInView` секции) + явный `play()` в ref-колбэке; подтверждено — 0 `<video>` в DOM до скролла, все 3 монтируются и играют только при попадании блока во вьюпорт.
- Проверено: `npm run build`/`lint` чистые. Playwright 1440/390 — все 3 видео `paused:false`/`readyState:4` после скролла на обеих ширинах; на mobile подтверждён порядок «один большой блок + два под ним» после фикса layout-бага.
- Unsplash больше нигде в `Story.tsx` не используется.

### v0.1.25 — 2026-09-12
- **Реальные часы работы от владельца: круглосуточно.** `Contact.tsx` — блок «Часы работы» заменил честный плейсхолдер «Уточняется» на «Круглосуточно» (тем же визуальным стилем, что «Адрес»/«Метро», а не курсивным serviceholder-стилем). `app/layout.tsx` JSON-LD `Restaurant` получил `openingHoursSpecification` (все 7 дней, `00:00`–`23:59`), meta-description дополнено «Работаем круглосуточно». `gastromania-tasks.md` — пункт «Режим работы ресторана» убран из списка недостающих данных.
- **Осознанно не тронуто:** слоты бронирования (`reservation/utils.ts`, `TIME_SLOTS` 19:00–21:30) — это окно организованных броней, отдельная бизнес-логика, не факт «когда ресторан открыт»; трогать без отдельного запроса не стал.
- Проверено: `npm run build`/`lint` чистые.

### v0.1.24 — 2026-09-12
- **`Gallery.tsx` переписан в живую видео-галерею** — владелец счёл предыдущую версию (3 видео-тайла + 3 статичных poster-кадра как "фото") слабой и случайной. Вместо статичных постеров — 8 коротких (5–6с) смысловых сцен, вырезанных из 4 исходных видео (`atmosphere`/`kitchen`/`desserts`/`coffee`), не половинным делением по времени, а по реальным границам сцен внутри монтажа (найдены через покадровый контакт-лист, 1 кадр/сек, на все 4 исходника). Точки in/out и итоговые размеры — в отдельном отчёте перед этим коммитом (в диалоге с владельцем). Клипы вырезаны из **исходных** (ещё не пережатых) файлов на Desktop, не из уже сжатых версий — чтобы не терять качество на двойном перекодировании.
- Каждый клип: H.264/`yuv420p`, `scale=720:1280`, `crf 26`, `preset medium`, `movflags +faststart`, **без звуковой дорожки** (`-an` — сайт всегда рендерит видео `muted`, звук не нужен). Плюс собственный JPG-постер на каждый (кадр внутри клипа, не всегда середина — для `kitchen-action` постер подобран вручную, чтобы не попасть на смазанный кадр в движении).
- Grid-композиция сохранена тем же словарём spans, что был (один большой 2×2-якорь, один высокий 1×2, один широкий 2×1, остальные 1×1) — просто расширена под 8 тайлов вместо 6.
- Убраны 3 неудачных статичных poster-тайла (Интерьер/Блюда/Кухня из v0.1.23.1) и 3 старых полноразмерных gallery-видео (`gastromania-atmosphere.mp4`/`-coffee.mp4`/`-desserts.mp4`, ~15MB) — заменены новыми короткими клипами в `public/videos/gallery/`. Старые файлы физически удалены (не использовались больше нигде — проверено `grep`).
- **Лайтбокс и вся фото-логика Gallery удалены** — с уходом последнего фото-тайла (`Image`, `AnimatePresence`, keyboard-nav, `useState`/`useEffect` для диалога) код стал бы мёртвым; тайлы теперь только видео, hover уже показывает подпись, отдельный полноэкранный режим не запрашивался.
- Ленивая загрузка не изменилась по механике — тот же `{inView && <video>}` (переиспользован `useInView({once:true})`) плюс явный `play()` в ref-колбэке (см. v0.1.23) — подтверждено: 0 `<video>`-элементов в DOM до скролла к галерее, все 8 монтируются и начинают воспроизведение только при попадании секции во вьюпорт, ни один не грузится при первой отрисовке `/`.
- Проверено: `npm run build`/`lint` чистые (по ходу поймал и убрал свой же неиспользуемый `ease` в новом `Gallery.tsx`). Playwright 1440/390 — все 8 видео подтверждены `paused:false`/`readyState:4` после скролла на обеих ширинах, до скролла — 0 видео-элементов. Console `400` (4 шт, Hero+Story Unsplash) — тот же известный sandbox-баг, не новый и не в рамках этой правки.
- **Не трогал Hero** (остаётся статичным изображением, как и просили) и backend/auth/orders/RLS/payment/Telegram.

### v0.1.23 — 2026-09-12
- **Реальный телефон `+7 995 555-22-27`** добавлен как кликабельная `tel:+79955552227` ссылка в четырёх местах: `Contact.tsx` (новый блок «Телефон» между «Адрес» и «Метро»), `Footer.tsx` (под адресом), `app/layout.tsx` JSON-LD (`Restaurant.telephone`), и новый `MobileContactCTA.tsx` — плавающая круглая кнопка-звонок, видна только на мобильной ширине (`lg:hidden`), фиксирована `bottom-6 right-6`, не влияет на layout других секций. Проверено: старый датский номер/email (`+45 12 34 56 78`, `reservations@gastronomia.dk`) оставался в `app/privacy/page.tsx` (не был задет в v0.1.21) — найден через `grep` по всему `app/`, убран; реального email от владельца по-прежнему нет, поэтому строку с email не заменил на выдумку, а убрал целиком, оставив только телефон.
- **5 реальных видео с Desktop** — сначала предварительный анализ (ffprobe: разрешение/fps/кодек/длительность/размер каждого исходника), затем перекодировка через `ffmpeg` (H.264/AAC, `scale=720:1280`, `crf 26`, `preset medium`, `movflags +faststart`) + JPG-постер на каждый — общий вес вырос с нативных ~78MB (при `crf 23`/1080p) до **~26MB на все 5** после A/B/C-тестирования настроек, что комфортно укладывается в `public/videos/` без раздувания деплоя (отдельный сторонний видео-хостинг не понадобился). Один общий уровень качества оказался достаточным — отдельный «облегчённый мобильный» вариант не понадобился.
  - **Важная самокоррекция по ходу задачи:** первый проход анализа (по 1-2 кадрам на файл) ошибочно принял 5 видео за случайные склейки разных сцен и решил, что `Кофейня_драфт.mp4` снята в чужом кафе — на основе этого владельцу был задан вопрос, и получен ответ «резать по сценам». После этого был сделан плотный покадровый (каждые 2 сек) контакт-лист по всем 5 файлам — выяснилось, что все они являются профессионально смонтированными промо-роликами с внутренними склейками по смыслу (не случайными), а `ESPRESSO.RU` на кадре — часть экосистемы Gastromania (подтверждено владельцем отдельно), и `Сопровождающее.mp4` содержит графику с реальным телефоном ресторана, подтверждающую подлинность. Ошибка была явно сообщена владельцу, вопрос переспрошен — итоговое решение: **использовать все 5 целиком, без нарезки**.
  - Размещение (подтверждено владельцем): `Сопровождающее.mp4` → `Contact.tsx`, заменил текстовый плейсхолдер карты (API карты по-прежнему не подключён — не было необходимости) на видео проезда/парковки с градиентом и подписью адреса поверх. `КУХНЯ_ШЕФ_Повар 1.mp4` → новая кинематографическая полоса в `Chef.tsx` между био шефа и сеткой «Команда кухни» (портрет-плейсхолдер «ЭД» не тронут — видео не выдаёт себя за портрет шефа). `Сопровождающее_Драфт 2.mp4`/`Кофейня_драфт.mp4`/`Десерты_драфт.mp4` → три плитки в `Gallery.tsx` (Атмосфера/Кофейня/Десерты), заменили 3 из 6 Unsplash-плейсхолдеров; оставшиеся 3 (Интерьер/Блюда/На кухне — реальных фото под них пока нет) остались как честные плейсхолдеры с пометкой «(временное фото)» в `alt`, только у заменённых тайлов эта пометка убрана.
  - **Производительность:** ни одно видео не грузится на `/` сразу — каждое смонтировано в DOM только через `{inView && <video>}` (переиспользован существующий паттерн `useInView({once:true})`), `preload="none"`, так что байты видео не запрашиваются, пока секция не появится во вьюпорте. Hero остаётся статичным изображением — подходящего видео для этого места не нашлось, по решению владельца.
  - **Найден и исправлен реальный баг:** атрибут `autoPlay` на условно смонтированном `<video>` ненадёжно срабатывал в headless Chromium (видео догружалось — `readyState 4` — но оставалось на паузе); подтверждено, что это не политика браузера (ручной `video.play()` через консоль срабатывал мгновенно без ошибки). Исправлено явным вызовом `el.play().catch(() => {})` в ref-колбэке на все 5 `<video>` — подстраховка поверх атрибута `autoPlay`, не замена (poster остаётся видимым фолбэком, если `play()` всё же будет заблокирован политикой реального браузера).
  - Лайтбокс `Gallery.tsx` (клик → полноэкранный просмотр) теперь работает только по фото-плиткам — видео-плитки уже показывают движение инлайн, отдельный полноэкранный режим для них не добавлялся.
- Проверено: `npm run build`/`lint` чистые (тот же единственный существующий warning в `Hero.tsx`, новых ошибок нет). Playwright на 1440px и 390px — все 5 видео подтверждены `paused:false`/`readyState:4` на обеих ширинах, 3 `tel:` ссылки найдены на странице, отсутствие layout shift проверено визуально, мобильная плавающая кнопка звонка видна и не перекрывает контент. Console-ошибки `400` на Unsplash — тот же известный sandbox-DNS баг из v0.1.21 (не связан с этой правкой, не реальный баг).
- **Не подключался платёжный провайдер, не трогался backend/auth/orders/RLS/Telegram.** Видео и постеры (~26MB суммарно) ещё не закоммичены — ждут подтверждения владельца перед commit.
