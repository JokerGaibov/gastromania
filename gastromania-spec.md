...
**Обновлено 2026-09-12 (main.md v0.1.21) — прогресс по пунктам:**
- ✅ Настоящее название, адрес, метро — есть и используются (ул. 7-я Кожуховская, 9, ТЦ «Мозаика», м. Дубровка)
- ⏳ Телефон, часы работы, реквизиты — **ещё не переданы**, на сайте честно помечено «уточняется» / не показывается
- ✅ Русский как основной язык — уже так с v0.1.1
- ⏳ Свои фотографии (интерьер, блюда, шеф) — **есть у владельца, но не переданы в проект.** Unsplash пока не убран — заменить, когда появятся реальные файлы
- ⏳ Реальное меню с ценами — **не передано.** `SignatureDishes.tsx` по-прежнему показывает вымышленные блюда несоответствующей кухни (нордическая, а не турецкая/ближневосточная) — см. `gastromania-tasks.md`, Блок 12, открытый вопрос
- ✅ Тексты о концепции и команде — переписаны на реальные факты (международная команда, био шеф-повара Эргюна Даи и команды кухни), без вымышленных дат/наград
